...
// // ── EMERGENCY — Counter Officer bypass: skip Survey/A.M.C./Payment, issue the Smart Card
// //    Certificate directly. Temporary shortcut for urgent situations only.
// //    DISABLED (28-10 request): backend route is commented out too. Kept here (commented,
// //    not deleted) in case Counter Officer emergency generation is needed again in future.
// // export async function emergencyIssueCertificate(applicationNo, { remarks } = {}) {
// //   try {
// //     const { data } = await apiClient.patch(`/applications/emergencyIssue/${applicationNo}`, { remarks });
// //     return data;
...
// // ── ID CARD — A.M.C. generates the Smart Card right after approving the application,
// //    WITHOUT payment (payment stays wired up in recordVendorPayment above for future scope). ──
// export async function generateIdCardByAmc(applicationNo, { remarks } = {}) {
//   try {
//     const { data } = await apiClient.patch(`/applications/generateIdCard/${applicationNo}`, { remarks });
//     return data;
//   } catch (err) {
//     return { success: false, message: err.response?.data?.message || "Could not generate the ID card." };
//   }
// }

// // ── BULK ID CARD — A.M.C. generates Smart Cards for many "A.M.C. Approved" applications at once ──
// export async function bulkGenerateIdCards(applicationNos) {
//   try {
//     const { data } = await apiClient.patch(`/applications/bulkGenerateIdCards`, { applicationNos });
//     return data;
//   } catch (err) {
//     return { success: false, message: err.response?.data?.message || "Bulk ID card generation failed. Please try again." };
//   }
// }
...
// ── Fetch vendor applications (role/ward filtered automatically by the backend, based on
// the logged-in user). Accepts ANY query param the backend's getAllApplications supports —
// status, search, hasCertificate, since, page, limit — and passes all of them straight
// through. Previously this only destructured status/page/limit by name, so newer filters
// (search, hasCertificate, since) were silently dropped here even though the backend and
// callers both supported them — e.g. Dashboard's "Smart Card Disbursed" count was fetching
// with NO filter at all as a result, returning the total application count instead. ──
export async function fetchVendorApplications(params = {}) {
  try {
    const { page = 1, limit = 100, ...rest } = params;
    const { data } = await apiClient.get("/applications/getAll", { params: { page, limit, ...rest } });
    return data; // { success, data, total, page, totalPages }
  } catch (err) {
    return {
      success: false,
