...
// ── `restricted` (NEW, optional, default false) — No Feriwala Area Geofencing.
//    When true: pin + coordinate badge turn red to visually flag that the
//    captured location falls inside a restricted "No Feriwala Area". ──
export default function MapPlaceholder({ latitude, longitude, onLocate, restricted = false }) {
  const [mode, setMode] = useState("Map");
  ...
  return (
    <div>
      ...
      >
        ...
        {/* No Feriwala Area Geofencing (NEW) — semi-transparent red overlay + badge when restricted */}
        {restricted && (
          <>
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-full">
          <FiMapPin
            ...
            size={30}
            fill={restricted ? "#EF4444" : "#F4A85B"}
            color={restricted ? "#7F1D1D" : "#B4611D"}
          />
        </div>

        <div
          className={`absolute bottom-3 left-3 rounded-lg px-2.5 py-1 text-[11px] font-semibold shadow-sm ${
            restricted ? "bg-red-600 text-white" : "bg-white/90 text-ink-700"
          }`}
        >
          {latitude && longitude ? `${latitude}, ${longitude}` : "Location not set"}
        </div>
