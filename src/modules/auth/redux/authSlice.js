...
//   registerVendor as registerVendorRequest,
// } from "../services/authService";
...
// // Vendor password login — identifier can be mobile number, application number, or username
// export const loginWithIdentifier = ({ identifier, password }) => async (dispatch) => {
//   dispatch(loginStart());
//   const result = await loginWithIdentifierRequest({ identifier, password });
//   if (result.success) {
//     dispatch(loginSuccess(result.session));
//     return { success: true };
//   }
...
// // Vendor self-registration — username + mobile number + password
// export const registerVendor = ({ fullName, userName, mobileNumber, password }) => async (dispatch) => {
//   dispatch(loginStart());
//   const result = await registerVendorRequest({ fullName, userName, mobileNumber, password });
//   if (result.success) {
//     dispatch(loginSuccess(result.session));
//     return { success: true };
//   }
...
export const verifyOtp = ({ mobileNo, otp, fullName, expectedRole }) => async (dispatch) => {
  dispatch(loginStart());
  const result = await verifyOtpRequest({ mobileNo, otp, fullName, expectedRole });
  if (result.success) {
    dispatch(loginSuccess(result.session));
    return { success: true };
  }
...
// Vendor/Staff password login — identifier can be mobile number or username
export const loginWithIdentifier = ({ identifier, password, expectedRole }) => async (dispatch) => {
  dispatch(loginStart());
  const result = await loginWithIdentifierRequest({ identifier, password, expectedRole });
  if (result.success) {
    dispatch(loginSuccess(result.session));
    return { success: true };
  }
