...
//   // ── EMERGENCY bypass — DISABLED (28-10 request). Kept commented, not deleted, in case
//   //    Counter Officer emergency ID generation is needed again in future. ──
//   // const EMERGENCY_ELIGIBLE_STATUSES = [
//   //   "Submitted",
//   //   "Sent Back to Counter Officer",
//   //   "Forwarded to Survey Officer",
...
//   // ── A.M.C. can generate the ID Card straight after approving — no payment required
//   //    right now (payment stays future scope). Only from "A.M.C. Approved". ──
//   const isAmc = user?.role === "A.M.C.";
//   const canAmcGenerateIdCard = isAmc && vendor.status === "A.M.C. Approved";
...
//   // ── EMERGENCY — DISABLED (28-10 request). Kept commented, not deleted. ──
//   // const handleEmergencyIssue = async () => {
//   //   setEmergencyError("");
//   //   setEmergencySubmitting(true);
//   //   const result = await emergencyIssueCertificate(applicationNo, { remarks });
...
//   // ── A.M.C.: generate the ID Card right after approval, without payment ──
...
//                 Kept commented, not deleted, in case it's needed again in future.
//             <p className="mb-2 flex items-center gap-1.5 text-xs font-bold text-amber-800">
//               <FiZap size={14} />skip Survey/A.M.C./Payment
//             </p>
//             <p className="mb-3 text-[11px] text-amber-700">
...
//             </span>
//             <StatusChip status={status} />
//             {vendor.status === "Certificate Issued" && (
//               <Link
//                 to={`/smart-card/${vendor.applicationNo}`}
//                 className="rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-600"
//               >
...
//             <div className="space-y-6">
//               <PersonalDetailsPanel vendor={vendor} />
//               <SurveyLocationPanel vendor={vendor} />
//             </div>
//           )}
//           {tab === "Business Info" && <BusinessInfoPanel vendor={vendor} />}
//           {tab === "Documents" && <DocumentsPanel vendor={vendor} />}
...
export default function VendorProfile() {
  ...
  // ── A.M.C. can generate the ID Card straight after approving — no payment required
  //    right now (payment stays future scope). From "A.M.C. Approved", and also
  //    "Payment Pending" as a rescue path for applications stuck there from before. ──
  const isAmc = user?.role === "A.M.C.";
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Payment — DISABLED for now (29-10 request). Kept commented, not deleted, so it can be
          switched back on later without losing this UI.
      {canVendorPay && (
        <Card>
          <h2 className="mb-1 font-display text-base font-bold text-ink-900">Payment Required</h2>
          <p className="mb-4 text-xs text-ink-500">
      ...
      <Card>
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-2 sm:items-end">
            ...
            <StatusChip status={status} />
            {canSeeSmartCard && vendor.status === "Certificate Issued" && (
              <Link
                to={`/smart-card/${vendor.applicationNo}`}
                className="rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-600"
              >
