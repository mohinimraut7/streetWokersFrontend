  ...
  // // Convert a native <input type="time"> 24-hour value ("HH:mm") into a
  // // 12-hour AM/PM string (e.g. "09:00" -> "09:00 AM") for storage/display.
  ...
  //   2: ["permanentAddress", "workingAddress", "ward"],
  ...
  //       workingAddress: data.workingAddress, // Working Address
  ...
  //       // Business Timing — combine the two time-picker values into a single readable
  //     // string, converted to 12-hour AM/PM format so it reads correctly everywhere
  //     // (backend, Application Approval, Smart Card / ID Card).
  //     const businessTiming =
  //       data.businessTimingFrom && data.businessTimingTo
  //         ? `${formatTime12h(data.businessTimingFrom)} - ${formatTime12h(data.businessTimingTo)}`
  //         : "";
  ...
  export default function VendorRegistration() {
    ...
    const handleSubmit = async () => {
      ...
      // Business Timing — combine the From/To Hour+Minute+AM-PM dropdown values
      // (Step3Business.jsx) into a single readable string. These dropdowns already
      // give AM/PM directly, so no 24-hour conversion is needed anymore.
      const businessTiming =
        data.businessTimingFromHour && data.businessTimingFromMinute && data.businessTimingFromPeriod &&
        data.businessTimingToHour && data.businessTimingToMinute && data.businessTimingToPeriod
          ? `${data.businessTimingFromHour}:${data.businessTimingFromMinute} ${data.businessTimingFromPeriod} - ${data.businessTimingToHour}:${data.businessTimingToMinute} ${data.businessTimingToPeriod}`
          : "";
