...
export default function ApplicationList() {
  ...
  // ── Only applications that have actually reached the A.M.C. stage (or beyond) belong in
  // this module. Previously this fetched the ENTIRE database first (limit:1 with no status
  // filter → total across ALL statuses including every Draft, e.g. 13,000+ records — then
  // re-fetched that whole count again) and only THEN filtered down to these 7 statuses on
  // the client. That's why this page was so slow. Now the status filter is applied on the
  // server (single request, comma-separated $in — see getAllApplications), so only the
  // relevant rows are ever transferred. ──
  ...
  const loadApplications = () => {
    setLoading(true);
    setError("");
    fetchVendorApplications({ status: RELEVANT_STATUSES.join(","), limit: 500 }).then((result) => {
      setLoading(false);
      if (!result.success) {
        setError(result.message || "Could not load applications.");
        return;
