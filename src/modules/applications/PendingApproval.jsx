...
export default function PendingApproval() {
  ...
  // ── "Forwarded to A.M.C." — backend already filters this to the logged-in officer's ward.
  // Single fetch now (no more "count first, then fetch everything" double round-trip) — a
  // ward's approval queue is never huge, so one call with a sane ceiling is enough and cuts
  // the network round-trips for this page in half. ──
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError("");
    fetchVendorApplications({ status: "Forwarded to A.M.C.", limit: 200 }).then((result) => {
      if (cancelled) return;
      setLoading(false);
      if (!result.success) {
        setError(result.message || "Could not load your approval queue.");
