...
//             {/* banner1.png is now shown at 100% x 100% (no zoom/crop) so
//                 the full artwork — including its top/edges — is always
//                 visible exactly as designed. overflow-hidden keeps it
//                 clipped to the header's own rounded corners only. Logo +
//                 text are lifted onto z-10 above the banner (z-0) so they
//                 always render on top of it regardless of stacking order. */}
//             <div
//               className="relative z-0 flex shrink-0 items-center gap-3 overflow-hidden pl-4 pr-3.5 pt-1.5 pb-3"
//               style={{
//                 backgroundImage: `url(${banner})`,
...
//               }}
//             >
//               <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-[1.5px] bg-gradient-to-r from-transparent via-[#D9BE68] to-transparent opacity-90" />

//               <div className="relative z-10 flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#ffffff] p-[2.5px] shadow-[0_0_0_1.5px_#C9A227]">
//                 <img src={logo} alt="VVCMC" className="h-full w-full rounded-full object-contain" />
//               </div>
...
//                    `flex flex-col justify-center` keeps both lines centered
//                    against the taller logo ring beside them. ── */}
//               <div className="relative z-10 flex min-w-0 flex-col justify-center gap-[4px] leading-tight">
//                 <p className="whitespace-nowrap text-[11.5px] font-bold uppercase tracking-[0.02em] text-[#ffffff]">
//                   Vasai-Virar City Municipal Corporation
//                 </p>
//                 <p className="whitespace-nowrap text-[12px] font-semibold leading-[14px] text-[#CA9D28]">
...
//             {/* Same fix as the front header above — banner shown at its
//                 natural 100% x 100% size (no zoom/crop), clipped only to
//                 the header's own rounded corners, with logo + text lifted
//                 above it on z-10. Keep this in sync with the front header
//                 if either is tweaked again. */}
//             <div
//               className="relative z-0 flex shrink-0 items-center gap-3 overflow-hidden pl-4 pr-3.5 pt-1.5 pb-3"
//               style={{
//                 backgroundImage: `url(${banner})`,
      ...
      <div className="flex flex-wrap items-start justify-center gap-10 px-1 py-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            ...
            {/* banner1.png is rendered as a real <img>, not a CSS
                background-image, and this header no longer clips with
                overflow-hidden. That's the actual root-cause fix: a
                background-image inside an overflow-hidden box was always
                going to have its bottom (the gold wave, the real design)
                sliced off the moment the box's fixed height was shorter
                than the image's natural proportions. An <img> instead
                renders at its own full natural aspect ratio (width 100%,
                height auto) and is simply allowed to extend past the
                header's bottom edge into the card body behind the fields —
                it sits at z-0, fully behind the logo/text (z-10) and behind
                the body's own content (which has its own stacking context
                below), so nothing outside the header ever overlaps visibly.
                The card's own OUTER rounded box still has overflow-hidden,
                so the image can never spill past the card's edges — it's
                only the header's own inner clipping that's been removed. */}
            {/* <div className="relative z-0 flex shrink-0 items-center gap-3 pl-4 pr-3.5 pt-1.5 pb-3">
              <img
                src={banner}
                alt=""
                aria-hidden="true"
             ...
             <div
  className="relative z-20 flex shrink-0 items-center gap-3 overflow-hidden pl-4 pr-3.5 pt-1.5 pb-3"
  style={{
    backgroundImage: `url(${banner})`,
    backgroundSize: "cover",
    backgroundPosition: "center",
  ...
  <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-[1.5px] bg-gradient-to-r from-transparent via-[#D9BE68] to-transparent opacity-90" />
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            ...
            {/* Same root-cause fix as the front header above — banner is a
                real <img> at its natural aspect ratio, header has no
                overflow-hidden, so nothing gets cropped. Keep this in sync
                with the front header if either is tweaked again. */}
            {/* <div className="relative z-0 flex shrink-0 items-center gap-3 pl-4 pr-3.5 pt-1.5 pb-3">
              <img
                src={banner}
