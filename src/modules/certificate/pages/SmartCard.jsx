...
/* =========================================================
   ADDRESS ROW — icon+label on their own line, value on the
   next line spanning the full card width (col-span-4). Used
   only for the two address fields, whose values are long
   enough to wrap — giving them the full row width (instead of
   squeezing into the narrow shared "value" grid column) keeps
   wrapped lines clean instead of cramped, with the dotted
   underline sitting cleanly below the last wrapped line.
========================================================= */
function AddressCardField({ icon: Icon, label, value }) {
  return (
    <div className="col-span-4 flex w-full flex-col gap-1">
      <div className="flex items-center gap-1.5">
        <span
          className="flex h-[15px] w-[15px] shrink-0 items-center justify-center rounded-full bg-[#E7EFEA] text-[#0B5D30]"
        >
          <Icon size={8.5} />
        </span>
        <span className="whitespace-nowrap text-[11.2px] font-bold leading-[14px] text-[#475569]">{label}</span>
      </div>
      <span className="min-w-0 whitespace-normal break-words border-b border-dotted border-[#C9A227] pb-[2px] pl-[21px] text-[12.3px] font-extrabold leading-[16px] text-[#0f172a]">
        {value || "\u00A0"}
      </span>
    </div>
  );
          ...
          >
            ...
            <div className="relative flex flex-1 flex-col items-center gap-2.5 px-4 pb-2.5 pt-3">
              ...
              >
                {backFields.map((f) =>
                  f.label === "व्यवसायाचा पत्ता" || f.label === "पत्ता" ? (
                    <AddressCardField key={f.label} icon={f.icon} label={f.label} value={f.value} />
                  ) : (
                    <IconCardField key={f.label} icon={f.icon} label={f.label} value={f.value} />
                  )
                )}
